...
#checking if a solution meets the criteria for validity, so it does not violate any of the constraints
def valid_solution(system):
    if system.heating_type == "geothermal" and system.size == "small":
        #it is very expensive to install geothermal heating, so someone living in a small house would not choose this option or may be unable to afford it
        return False
    if system.heating_type == "oil" and system.control_type == "energy_saver":
        #oil-based heating systems are less effective and cost more money when they turn on and off frequently
        #so it would not make sense to use an energy saving control strategy, which would cause the system to turn on and off more frequently
        return False
    if system.insulation == "poor" and system.target_temp > 20:
        #a house with poor insulation would likely not be able to maintain a very high target temperature
        #because too much heat would be lost, so it would not make sense to have a target temperature above 20 degrees
        return False
    return True
...
def random_solution():
    #3 (Hard constraints)
    while True: #keep generating random solutions until we get a valid one
        ...
        system = Heating_System(size, insulation, heating_type, target_temp, control_type)

        if(valid_solution(system)): #only valid solutions will be returned, so we know that invalid solutions are not possible
            return system
...
def genetic_algorithm(size, generations, temps):
    ...
    for gen in range(generations):
        ...
        for i in range(int(size / 2)):  # creating new children using crossover, only creates 1/2 the size to leave room for the fit parents
            ...
            while not valid_solution(child): # if crossover creates an invalid solution, keep creating new children until we get a valid one
                child = crossover(parents[0], parents[1])
            offspring.append(child)

        for c in range(len(offspring)): # introduce random mutation to next generation, with validation making sure constraints are met
            mutate(offspring[i], 0.1)
            while not valid_solution(offspring[i]): #if mutation causes an invalid solution, keep mutating until we get a valid solution
                offspring[i] = random_solution()
...
comparison = pd.DataFrame({ #saving constrained results in a dataframe to compare with unconstrained results
    "Generation": range(1, len(best) + 1),
    "Best Fitness Constrained": best,
    "Mean Fitness Constrained": mean
})
comparison.to_csv("constrained_fitness_results.csv", index=False) #saving constrained results in a csv file
